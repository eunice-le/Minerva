...
# Read a real decision-memo that discusses matching design and analysis:
# https://docs.google.com/document/d/1SkGRu1lipC-Z4-wVFRmQrKd7SIGRcDOJN3pCz-voP_s/edit?usp=sharing

# And read the code, as well.

#### Your task is to change the code, specifically the matching algorithm.
#### You won't be changing all the code.
#### You will only be changing the part that appears after the following
...
### MODIFIED CODE ###
#extract the first two digits of the district code
foo$district_code2 <- foo$district_code%/% 100 
foo$district_code2[which(is.na(foo$district_code2) == TRUE)] <- -9999999 

#eliminate NA date of births 
which.have.NAs <- which(is.na(foo$date_of_birth == TRUE)) 
foo <- foo[-which.have.NAs, ]
#change the Date variable into numeric to calculate the standard deviation 
foo$date_of_birth <- as.numeric(foo$date_of_birth) 
#calculating the caliper for 'date_of_birth'
365/sd(foo$date_of_birth) #0.066

#in covariate selection: switch postal_code1 for district_code2
...
# Part (A): In the body of your submitted assignment (not in a link), submit the code you added/modified AND 
# provide your your Match() and MatchBalance() output (which must now also additionally show balance on the district code variable).

# Part (B): Provide a few sentences talking about the number of treated units dropped, and a few more sentences talking 
# about the balance obtained.
# Be sure to assess the balance obtained. Be sure to also explain what it means to match exactly on the first 2 digits of
# the district code. 
# Hint 1: https://m.nomor.net/_kodepos.php?_i=kode-wilayah&_en=ENGLISH#Kecamatan and
# Hint 2: https://translate.google.com/translate?hl=en&sl=id&u=https://id.wikipedia.org/wiki/Daftar_kecamatan_dan_kelurahan_di_Indonesia&prev=search
# You may also find this spreadsheet enumerating the districts helpful (but I doubt it): 
#https://docs.google.com/spreadsheets/d/e/2PACX-1vRDzst7Ceo2KvMHOLw-hghXChiEmkFErEaWTjq6bS_bT3NBDM5hOIuxjKayMi6PJVzp1tyDzV0DCP8v/pub?gid=3552843&single=true&output=csv
