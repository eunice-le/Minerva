...
# (var 4) gender is 3 levels: men, women, and business entity ("LAKI-LAKI"   "PEREMPUAN"   "BADANUSAHA")
...
#extract the first 2 digits from the district code 
foo$district_code2 <- foo$district_code%/% 100 #dividing 4-digit district code by 100 and keeps only the interger part 
foo$district_code2[which(is.na(foo$district_code2) == TRUE)] <- -9999999

#change date_of_birth to Date format
foo$date_of_birth <- as.Date(as.character(foo$date_of_birth)) 
#eliminate the rows with NAs in the date_of_birth column
which.NAs <- which(is.na(foo$date_of_birth == TRUE)) 
foo <- foo[-which.NAs, ]
#switch date_of_birth from Date format to Numeric format 
foo$date_of_birth <- as.numeric(foo$date_of_birth)
#calculating the caliper for date_of_birth - to use in GenMatch 
clp <- 365/sd(foo$date_of_birth) #result = 0.107

#defining covariates for GenMatch 
#replace postal code with district code 
#replace R_date_of_birth with date_of_birth 
X <- cbind(foo$date_of_birth, foo$gender, foo$marital_status,
...
#set up the exact matching for distric_code2 
#set the caliper for date_of_birth to 365 days or 0.107 standard deviations
#the calipers for other covariates are set to 1e16, which is really large = virtually no limit 
...
#now that we have weight from GenMatch
#we set weight.matrix as genout 
...
mb <- MatchBalance(foo$treat~
                     foo$date_of_birth + foo$gender + foo$marital_status +
                     foo$education + foo$occupation + foo$district_code2 +
                     foo$worker + foo$capital + foo$credit_proposal +
                     foo$worker_transformed + foo$capital_transformed + 
                     foo$credit_proposal_transformed +
